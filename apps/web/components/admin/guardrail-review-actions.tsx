...
import { setGuardrailEventStatus } from '@/lib/actions/review-feedback';

interface Props {
  eventId: string;
}

export function GuardrailReviewActions({ eventId }: Props) {
  const [pending, startTransition] = useTransition();

  function act(status: 'reviewed' | 'dismissed'): void {
    startTransition(async () => {
      const result = await setGuardrailEventStatus({ id: eventId, status });
      if (result.ok) {
